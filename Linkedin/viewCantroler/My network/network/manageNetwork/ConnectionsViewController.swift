...
class ConnectionsViewController: UIViewController {
    ...
    func registerNib() {
        connectionsTableView.register(UINib(nibName: "ConnectionTableViewCell", bundle: nil), forCellReuseIdentifier: "ConnectionTableViewCell")
    }
}
...
extension ConnectionsViewController: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell: ConnectionTableViewCell = connectionsTableView.dequeueReusableCell(withIdentifier: "ConnectionTableViewCell", for: indexPath) as! ConnectionTableViewCell
        cell.profileName.text = "\(arrProfileName[indexPath.row])"
        cell.post.image = UIImage(named: arrImage[indexPath.row])
        cell.connctionDay.text = "\(arrdiscription[indexPath.row])"
        return cell
    }
}
